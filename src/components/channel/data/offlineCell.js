...
import { DataOfflineOrderConfig, DataSortType } from 'constants/channel'
import Style from './offlineCell.less'
  ...
  render () {
    ...
    return <ScrollOnTop className='scroll-on-top-wap'><table className='list-title'>
      <tbody>
        <tr>
          <td>设备编号</td>
          {DataOfflineOrderConfig.map((item, i) => {
      ...
      <div className='device-cell'>
        <Link className='shop clearfix' to={'/shops/' + this.props.shopId}>
          <div className='install'>
            ...
            <p className='install-num'>{this.props.nickName ? `员工：${this.props.nickName}` : null} {`设备总数：${this.props.deviceCount || 0}`} {`离线设备：${this.props.sumDeviceCount || 0}`}</p>
          </div>
      ...
      <table>
        <tbody>
          <tr onClick={() => this.props.triggerChangeOpenStatus(this.props.index)}>
            <td>门店合计：</td>
            <td className='order'>{this.props.todayofflineTimeCount || 0}</td>
            <td className='order'>{this.props.todayScanNumCount || 0}</td>
            <td className='order'>{this.props.totalSuccessOrderCount || 0}</td>
            <td className='order'>{this.props.totalRefundOrderCount || 0}</td>
          </tr>
            ...
            return (
              <tr className='tr' key={i}>
                <td>
                  ...
                  <p>{ batteryInDevice(item.battery) ? <span className='green'>{`电量：${item.battery}%`}</span> : null} {!batteryInDevice(item.battery) && (item.status === 0 || item.status === 1) ? <span className='green'>交流电</span> : null}</p>
                  {item.reason && item.reason.length ? <p className='battery-use'>离线原因：{item.reason.map((o, i) => <span key={i}>{o}</span>)}</p> : null}
                </td>
                <td className='order'>{item.lastOfflineTimeHour || 0}</td>
                <td className='order'>{item.todayScanNum || 0}</td>
                <td className='order'><Link to={'/shop/device/orderList?' + params({deviceNo: item.deviceNo, shopName: _this.props.shopName})}>{item.successOrder || 0}</Link></td>
                <td className='order'><Link to={'/shop/device/orderList?' + params({deviceNo: item.deviceNo, shopName: _this.props.shopName})}>{item.refundOrder || 0}</Link></td>
              </tr>
            )
          })
          }
