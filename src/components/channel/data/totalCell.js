...
import { DataTotalOrderConfig, DataSortType } from 'constants/channel'
...
class Title extends PureComponent {
  ...
  render () {
    ...
    return <ScrollOnTop className='list-title'><table className='list-title'>
      <tbody>
        <tr>
          <td>设备编号</td>
          {DataTotalOrderConfig.map((item, i) => {
      ...
      <table>
        <tbody>
          <tr onClick={() => this.props.triggerChangeOpenStatus(this.props.index)}>
            <td>门店合计：</td>
            <td className='order'>{this.props.totalScanNumCount || 0}</td>
            <td className='order'>{this.props.totalCreateOrderCount || 0}</td>
            <td className='order'>{this.props.totalSuccessOrderCount || 0}</td>
            <td className='order'>{this.props.totalRefundOrderCount || 0}</td>
            <td className='order'>{this.props.totaldeviceAverageOrder || 0}</td>
          </tr>
          {this.props.open && this.props.devices.map((item, i) => {
            ...
            return (
              <tr className='tr' key={i}>
                ...
                <td className='order'>{item.scanNum || 0}</td>
                <td className='order'>{item.createOrder || 0}</td>
                ...
                <td className='order'>{item.totalAverageOrder || 0}</td>
              </tr>
            )
          })
          }
