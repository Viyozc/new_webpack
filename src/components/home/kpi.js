  ...
  _options (user, data, selectedDate) {
    ...
    switch (role) {
        ...
        if (isThisMoth) {
          options = options.concat([
            <div key={0} className='analyse clearfix'>
              <DeviceCell title='累计离线设备' value={data.offlineDevice} onClick={this._bindLocationTo.bind(this, '/shop/device/offlineList')} />
              <DeviceCell title='今日订单' value={data.todayOrder} onClick={this._bindLocationTo.bind(this, '/shop/orderList')} />
              <DeviceCell title='累计安装设备' value={data.totalDevice} onClick={this._bindLocationTo.bind(this, '/shop/device/totalList')} />
            </div>,
        ...
        options = options.concat([
          <div key={2} className='kpi'>
            ...
            <div className='data need'>
              <div onClick={this._bindLocationTo.bind(this, '/shop/avgDayOrderList?selectedDate=' + this.props.selectedDate)} className='shop-tab'>
                <p>本月<br />日均订单数 {<i className='dianfont icon-xuanze' />}</p>
                <p>{data.thisMonthAverageDayOrder}</p>
              </div>
              <div onClick={this._bindLocationTo.bind(this, '/shop/device/avgDayOnlineRateList?selectedDate=' + this.props.selectedDate)} className='shop-tab'>
                <p>本月设备<br />日均在线率 {<i className='dianfont icon-xuanze' />}</p>
                <p>{`${data.thisMonthDeviceAverageDayOnline || 0}%`}</p>
              </div>
              <div onClick={this._bindLocationTo.bind(this, '/shop/device/recycleList?selectedDate=' + this.props.selectedDate)} className='shop-tab'>
                <p>本月<br />回收设备数 {<i className='dianfont icon-xuanze' />}</p>
                <p>{data.thisMonthRecycleDeviceNum}</p>
              </div>
              <div onClick={role !== 'agentSellerManager' ? this._bindLocationTo.bind(this, '/shop/device/list?' + params({title: '当前考核期数据', checkStatus: 1, selectedDate: this.props.selectedDate})) : null} className='shop-tab'>
                <p>当前考核期达标设备数 {role !== 'agentSellerManager' ? <i className='dianfont icon-xuanze' /> : null}</p>
                <p>{data.thisMonthDeviceAchieve}</p>
              </div>
              <div onClick={role !== 'agentSellerManager' ? this._bindLocationTo.bind(this, '/shop/device/list?' + params({title: '当前考核期数据', checkStatus: 2, selectedDate: this.props.selectedDate})) : null} className='shop-tab'>
              ...
              <div onClick={role !== 'agentSellerManager' ? this._bindLocationTo.bind(this, '/shop/device/list?' + params({title: '下个考核期数据', checkStatus: 1, selectedDate: selectedNextDate})) : null} className='shop-tab'>
                <p>下个考核期达标设备数 {role !== 'agentSellerManager' ? <i className='dianfont icon-xuanze' /> : null}</p>
                <p>{data.nextMonthDeviceAchieve}</p>
              </div>
              <div onClick={role !== 'agentSellerManager' ? this._bindLocationTo.bind(this, '/shop/device/list?' + params({title: '下个考核期数据', checkStatus: 2, selectedDate: selectedNextDate})) : null} className='shop-tab'>
      ...
      case 'installer':
        ...
        options = [
          <div key={0} className='analyse clearfix'>
            <DeviceCell title='已安装' value={data['installed']} onClick={() => this.props.onLocationTo('/shops?activeTab=2&roleIndex=1')} />
            <DeviceCell title='已维修' value={data['repaired'] || 0} onClick={() => this.props.onLocationTo('/workOrder/repair?activeTab=1')} />
          </div>
        ]
        break
      default:
