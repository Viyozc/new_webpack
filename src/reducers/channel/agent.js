...
let IndexPage = (state = null, action) => {
  switch (action.type) {
    case ActionTypes.CHANNEL_AGENT_GET_LIST:
...
let SearchPage = (state = null, action) => {
  switch (action.type) {
    case ActionTypes.CHANNEL_AGENT_GET_SEARCH_LIST:
      switch (action.status) {
        case STATUS_REQUEST:
          return assign({}, state, {list: action.requestData.offset === 0 ? null : state.list})
        case STATUS_SUCCESS:
...
let CreatePage = (state = {editInfo: {}}, action) => {
  switch (action.type) {
    case ActionTypes.CHANNEL_AGENT_TRIGGER_UPDATE_AGENT_INFO:
      state.editInfo[action.payload.key] = action.payload.value
      return assign({}, state, {editInfo: assign({}, state.editInfo)})
    case ActionTypes.CHANNEL_AGENT_TRIGGER_CLEAN_EDIT_INFO:
      return assign({}, state, {editInfo: {}})
    case ActionTypes.CHANNEL_AGENT_GET_AGENT_INFO:
      switch (action.status) {
        case STATUS_SUCCESS:
          action.payload.agentStartTime = action.payload.agentStartTime
          action.payload.agentEndTime = action.payload.agentEndTime
          action.payload.agent = {key: action.payload.level && action.payload.level.levelValue, value: action.payload.level && action.payload.level.levelKey}
          return assign({}, state, {editInfo: action.payload})
      }
      return state
    case ActionTypes.CHANNEL_AGENT_POST_SAVE_AGENT:
    case ActionTypes.CHANNEL_AGENT_POST_UPDATE_AGENT:
      switch (action.status) {
        case STATUS_REQUEST:
