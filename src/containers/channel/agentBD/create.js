...
class CreateContainer extends Component {
  ...
  componentDidMount () {
    if (this.props.location.query.id) {
      Bridge.setNavTitle('编辑BD代理')
    } else {
      Bridge.setNavTitle('添加BD代理')
    }
  ...
  render () {
    ...
    if (!this.props.provincesAndCities && !this.props.error) {
      return <Loading />
    }
    if (!this.props.provincesAndCities && this.props.error) {
      return <Error>{this.props.error.message || '未知错误'}</Error>
          ...
          <div className='openinghours '>
            ...
            <div className='openinghours-panel'>
              <ul className='openinghours-content'>
                <li>
                  {this.props.location.query.id ? <div className='times disabled'>
                    <p className='start'>{this.props.editInfo.agentStartTime || '选择开始时间'}</p>
                    <p className='to'>至</p>
                    <p className='end'>{this.props.editInfo.agentEndTime || '选择结束时间'}</p></div>
                    : <div className='times'>
                      <p className='start'
...
function mapStateToProps (state, ownProps) {
  ...
  return {
    ...
    editInfo: state.channelAgentBDCreatePage && state.channelAgentBDCreatePage.editInfo,
    fetch: state.channelAgentBDCreatePage && state.channelAgentBDCreatePage.fetch
  }
}
