...
class TodayNewInstallDeviceContainer extends Component {
  ...
  componentDidMount () {
    ...
    if (!this.props.list && this.props.location.query.shopSubType) {
      ...
      this.props.actions.fetchTodayNewInstallDeviceList(assign({offset: 0, pageSize: PAGE_SIZE}, this.props.location.query))
    }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!isEqual(this.props.location.query, nextProps.location.query)) {
      this.props.actions.fetchTodayNewInstallDeviceList(assign({offset: 0, pageSize: PAGE_SIZE}, nextProps.location.query))
    }
    ...
    return (
      <Pagination complete={this.props.complete} paging={this.props.paging} location={this.props.location} onPaging={() => { this._paging(this.props.list) }} data={this.props.list} size={PAGE_SIZE}>
        ...
        this.props.list && this.props.list.map((item, i) => {
          return <Cell key={i} index={i} {...item} triggerChangeOpenStatus={this.props.actions.triggerTodayNewInstallDeviceChangeOpenStatus} />
        })
      }
      </Pagination>
    )
  ...
  _paging (list) {
    this.props.actions.fetchTodayNewInstallDeviceList(assign({offset: list.length, pageSize: PAGE_SIZE}, this.props.location.query))
  }
  _tab (status) {
...
function mapStateToProps (state, ownProps) {
  ...
  return {
    ...
    districts,
    members,
    list: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.list,
    complete: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.complete,
    paging: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.paging,
    deviceCount: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.deviceCount,
    shopCount: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.shopCount,
    successOrder: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.successOrder,
    createOrder: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.createOrder,
    refundOrder: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.refundOrder,
    scanNum: state.channelDataTodayNewInstallDevicePage && state.channelDataTodayNewInstallDevicePage.scanNum
  }
}
