...
import { Title, Cell } from 'components/channel/data/totalCell'
import { router, limitFontSize } from 'utils'
import Search from 'components/common/search'
import * as sessionStorage from 'utils/sessionStorage'
import { DataTabConfig, DataTotalOrderConfig, DataSortType } from 'constants/channel'
const PAGE_SIZE = 10

class TotalInstallShopContainer extends Component {
  ...
  componentDidMount () {
    switch (this.props.location.pathname.substr(this.props.location.pathname.lastIndexOf('/') + 1)) {
      case 'totalInstallShop':
        Bridge.setNavTitle('累计安装门店数')
        break
      case 'totalInstallDevice':
        Bridge.setNavTitle('累计安装设备数')
        break
      case 'totalSuccessOrder':
        Bridge.setNavTitle('累计成功订单数')
        break
    }
    if (!this.props.list && this.props.location.query.shopSubType) {
      ...
      this.props.actions.fetchTotalInstallShopList(assign({offset: 0, pageSize: PAGE_SIZE}, this.props.location.query))
    }
            ...
            if (this.props.location.query.shopSubType) {
              ...
              router.replace({
                ...
                query: assign(this.props.location.query, {
                  ...
                  dataSortField: DataTotalOrderConfig[4].key,
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!isEqual(this.props.location.query, nextProps.location.query)) {
      this.props.actions.fetchTotalInstallShopList(assign({offset: 0, pageSize: PAGE_SIZE}, nextProps.location.query))
    }
    ...
    return (
      <Pagination complete={this.props.complete} paging={this.props.paging} location={this.props.location} onPaging={() => { this._paging(this.props.list) }} data={this.props.list} size={PAGE_SIZE}>
        ...
        this.props.list && this.props.list.map((item, i) => {
          return <Cell key={i} index={i} {...item} triggerChangeOpenStatus={this.props.actions.triggerTotalInstallShopChangeOpenStatus} />
        })
      }
      </Pagination>
    )
  ...
  _paging (list) {
    this.props.actions.fetchTotalInstallShopList(assign({offset: list.length, pageSize: PAGE_SIZE}, this.props.location.query))
  }
  _tab (status) {
...
function mapStateToProps (state, ownProps) {
  ...
  return {
    ...
    districts,
    members,
    list: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.list,
    complete: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.complete,
    paging: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.paging,
    deviceCount: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.deviceCount,
    shopCount: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.shopCount,
    successOrder: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.successOrder,
    createOrder: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.createOrder,
    refundOrder: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.refundOrder,
    scanNum: state.channelDataTotalInstallShopPage && state.channelDataTotalInstallShopPage.scanNum
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(TotalInstallShopContainer)
