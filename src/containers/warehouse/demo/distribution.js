...
class RecyleContainer extends Component {
  ...
  componentWillMount () {
    Bridge.setNavTitle('演示出库')
    Style.use()
  }
  componentDidMount () {
    this.props.actions.fetchDemoList(TabConfig[this.props.location.query.activeTab || 0].status, 0, PAGE_SIZE)
    this.props.actions.fetchDemoCount()
  }
  render () {
            ...
            : <Pagination data={list} onPaging={this.paging.bind(this)} size={PAGE_SIZE}>
              {
                list && list.length ? list.map((o, i) => {
                  return <Item isDemo={1} showCheckList={this.showCheckList.bind(this)} hideCheckList={this.hideCheckList.bind(this)} getRepairDetail={this.props.actions.repairDetail} status={Number(this.props.location.query.activeTab || 0)} key={i} activeTab={TabConfig[this.props.location.query.activeTab || 0]} {...o} />
  ...
  paging () {
    this.props.actions.fetchDemoList(TabConfig[this.props.location.query.activeTab || 0].status, this.props.data.list.length, PAGE_SIZE)
  }
  showCheckList (id) {
  ...
  _check (event) {
    this.props.actions.claimDemoTask({id: this.state.checkId})
  ...
  componentDidUpdate (prevProps, prevState) {
    if (prevProps.location.query.activeTab !== this.props.location.query.activeTab) {
      this.props.actions.fetchDemoList(TabConfig[this.props.location.query.activeTab].status, 0, PAGE_SIZE)
      this.props.actions.fetchDemoCount()
    }
  }
