...
class ShopCreateContainer extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.fetch === 'request' && nextProps.fetch === 'success') {
      ...
      //   router.goBack()
      // }
      this._locationToAddTwo(this.state.shopId)
    }
      ...
      <div className='shop-create'>
        ...
        <Cells>
          <Cell icon='gongsi cell-icon-color' others={{onClick: (e) => { this._handleSelectMerchantName(e) }}}>
            <CellBody style={{width: '50%', marginRight: '30px'}}>
              <div>{this.props.form.merchantName || HOLDER_SELECT_COMPANY}</div>
            </CellBody>
            <CellFooter>
          ...
          <Cell icon='mendian cell-icon-color' >
            <CellBody style={{width: '50%', marginRight: '30px'}}>
              <div className='radio-cell'>{this.props.form.address && this._renderAddress() || HOLDER_SELECT_ADDRESS}</div>
            </CellBody>
          </Cell>
          <Line />
        ...
        <Cells>
          <Cell icon='fenlei cell-icon-color'>
            <CellBody style={{width: '50%', marginRight: '30px'}}>
              <div className='radio-cell'>{this.props.form.type && this.props.form.type.name || HOLDER_SELECT_TYPE}</div>
            </CellBody>
          </Cell>
          <Line />
        ...
        <Cell className='uploader-wrapper'>
          {this.props.form.picUrl ? this._renderImage() : this._renderUploader()}
          {this.props.form.contractPicUrl ? this._renderContractImage() : this._renderUploader()}
        </Cell>
        <a className='button save' href='javascript: void(0)' onClick={() => { this._handleClickSave(0) }} >保存并下一步</a>
        <AlertChoseDeviceType Bridge={Bridge} closeOpc={() => { this.setState({isShowOpc: false}) }}
          isShowOpc={this.state.isShowOpc} router={router} shopId={this.props.location.query.shopId} />
      </div>
    )
  }
  ...
  _renderImage () {
    return <div onClick={(e) => { this._handleClickUploader(e) }} >
      <div className='titleImage'>店面照片</div>
      <img className='image' src={this.props.form.picUrl} />
    </div>
  }
  _renderContractImage () {
    return <div >
      <div className='titleImage'>合同照片</div>
      <img className='image' src={this.props.form.contractPicUrl} />
    </div>
  }
  _renderUploader () {
    return (
