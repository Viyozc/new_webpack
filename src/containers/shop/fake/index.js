...
import * as actions from 'actions/shop/fake'
...
class CancelSignContainer extends Component {
  ...
  componentWillMount () {
    Style.use()
    Bridge.setNavTitle('无法安装原因')
    this.props.actions.getOptions(12)
  }
  ...
  _confirm (e) {
    ...
    if (this.state.option === -1) {
      return Toast.show('请选择无法安装原因')
    }

    if (parseInt(this.state.option) === 0 && this.state.reasonTxt === '') {
      return Toast.show('请输入无法安装原因')
    }

    if (!varify.limitWordLength(this.state.reasonTxt, 100)) {
      return Toast.show('最多只能输入50个字')
    }

    this.props.actions.fakeBD({
      shopId: this.props.params.id,
      option: this.state.option,
      unInstallReason: this.state.reasonTxt
    })
  }
}
...
function mapStateToProps (state, ownProps) {
  return {
    ...
    submitStatus: state.shopFakeBD && state.shopFakeBD.submitStatus
  }
}
