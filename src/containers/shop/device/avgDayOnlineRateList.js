...
import Style from './avgDayOnlineRateList.less'
import Loading from 'components/common/loading'
import Error from 'components/common/error'
import Notfound from 'components/common/notfound'
import DoubleSelect from 'components/common/doubleSelect'
import { clean } from 'actions/errorMessage'
import * as actions from 'actions/shop/device/avgDayOnlineRateList'
import * as commonActions from 'actions/common'
import assign from 'lodash/assign'
import { router } from 'utils'
import Search from 'components/common/search'

class AvgDayOnlineRateListContainer extends Component {
  ...
  componentDidMount () {
    Bridge.setNavTitle('本月设备日均在线率')
    if (this.props.location.query.rightChoseValue) {
      this.props.actions.fetchDataSellerShopDeviceAvgDayOnlineRateList({
    ...
    return (
      <div>
        ...
        <div className='total-num'>
          ...
          <p>本月设备日均在线率</p>
          <p>考核在线率(<span className='kpi'>≥90%</span>)</p>
        </div>
        ...
        {this.props.list.map((item, i) => {
          return <div key={i} className='tr clearfix' >
            ...
            <div>{item.onlineDeviceNum}</div>
            <div>{`${item.averageDayOnline}%`}</div>
          </div>
        })}
      </div>
    )
...
function mapStateToProps (state, ownProps) {
  return {
    ...
    list: state.shopDeviceAvgDayOnlineRateList && state.shopDeviceAvgDayOnlineRateList.list,
    averageDay: state.shopDeviceAvgDayOnlineRateList && state.shopDeviceAvgDayOnlineRateList.averageDay
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(AvgDayOnlineRateListContainer)
