...
import * as actions from 'actions/shop/device/offlineList'
...
class OfflineListContainer extends Component {
  ...
  componentDidMount () {
    ...
    if (sessionStorage.getItem(`${this.props.location.pathname}_search`) !== this.props.location.search) {
      if (this.props.location.query.rightChoseValue || this.props.location.query.orderType) {
        this.props.actions.fetchDataSellerShopOfflineList({
          offset: 0,
          pageSize: PAGE_SIZE,
    ...
    if (this.props.location.query.rightChoseValue || this.props.location.query.orderType) {
      !this.props.list && this.props.actions.fetchDataSellerShopOfflineList({
        offset: 0,
        pageSize: PAGE_SIZE,
        memberId: this.props.location.query.rightChoseValue,
        cityCode: this.props.location.query.leftChoseValue,
  ...
  _renderList () {
    ...
    if (this.props.list && this.props.list.length === 0) {
      return <Notfound>暂无离线设备</Notfound>
    }
    return (
...
function mapStateToProps (state, ownProps) {
  return {
    ...
    list: state.shopDeviceOfflineList && state.shopDeviceOfflineList.list,
    deviceCount: state.shopDeviceOfflineList && state.shopDeviceOfflineList.deviceCount,
    shopCount: state.shopDeviceOfflineList && state.shopDeviceOfflineList.shopCount,
    successOrder: state.shopDeviceOfflineList && state.shopDeviceOfflineList.successOrder
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(OfflineListContainer)
