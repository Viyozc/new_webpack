...
class ReasonContainer extends Component {
  ...
  componentWillMount () {
    Bridge.setNavTitle('拒绝原因')
    Style.use()
  }
  componentDidMount () {
    this.props.actions.fetchGetShopExamineReasons()
  }
  render () {
    ...
    return <div className='result'>
      <ul>
        {
        this.props.reasons.map((o, i) => {
          return <li onClick={this._bindSelectOption.bind(this, o.id)} key={i}>{o.value}<span className={`radio ${this.state.selectedIndex === o.id ? 'selected' : ''}`} /></li>
        })
      }
      </ul>
  ...
  _configure () {
    if (typeof this.state.selectedIndex !== 'number') return Toast.show('请选择拒绝原因')
    NProgress.start()
    this.props.actions.fetchShopExamineApprovalRefuse({id: this.props.params.id, reasonId: this.state.selectedIndex, reasonDes: this.state.reason})
  }
  ...
  componentWillUnmount () {
    this.props.actions.cleanShopExamineReason()
    Style.unuse()
  }
}
...
function mapStateToProps (state, ownProps) {
  return {
    ...
    reasons: state.shopExamineReason && state.shopExamineReason.reasons,
    fetch: state.shopExamineReason && state.shopExamineReason.fetch
  }
}
