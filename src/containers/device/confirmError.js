...
class ConfirmContainer extends Component {
  ...
  _renderSuccess () {
    ...
    return (
      <div className='confirm'>
        ...
        <Button fixed onClick={() => { if (!this.state.isRequest) { this._confirm() } }}>确认</Button>
        {
          this.state.success
            ? <div className='added'>
  ...
  _renderFail () {
    ...
    return (
      <div className='fail'>
        <p className='title red'>设备已绑定其他门店</p>
        ...
        <div className='value fail'>
          ...
          <p>设备二维码编号: {this.props.location.query.deviceNo}</p>
        </div>
        <p className='info'>当前安装信息</p>
        <div className='value'>
          <p>门店名称: {this.props.shopInfo.deviceInfo.shopName}</p>
          <p>设备编码: {this.props.location.query.deviceNo}</p>
        </div>
        <Button type='white' className='giveup' fixed onClick={() => { router.goBack() }}>二维码信息有误，重新扫描</Button>
        <Button fixed onClick={() => router.push(`/shops/${this.props.location.query.shopId}`)}>返回门店详情</Button>
      </div>
    )
  }
