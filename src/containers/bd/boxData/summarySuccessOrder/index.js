  ...
  render () {
    return (
      <div className='summary-wap'>
        ...
        ? this.state.tabIndex === 'ORDER_MONEY'
            ? <div className='total'> {/* 订单金额合计 */}
              ...
              <ul className='clearfix'>
                ...
                <li>创建订单金额：{(parseFloat(this.props.summaryData.totalOrderMoney || 0) / 100).toFixed(2)}</li>
                <li>成功订单金额：{(parseFloat(this.props.summaryData.totalSuccessOrderMoney || 0) / 100).toFixed(2)}</li>
                <li>退款订单金额：{(parseFloat(this.props.summaryData.totalRefundOrderMoney || 0) / 100).toFixed(2)}</li>
              </ul>
            </div>
            : <div className='total'> {/* 订单数合计 */}
              <a href='/data/help' className='data-help'>?</a>
              <p className='total-title'>合计：</p>
              <ul className='clearfix'>
                <li>门店数：{this.props.summaryData.totalInstallShop || 0}</li>
        ...
        {this.state.tabIndex === 'ORDER_MONEY' ? <ScrollOnTop className="scroll-on-top-wap"><div className='table-head'> {/* 订单金额表格标题 */}
          ...
          <span className='th wid03'>
            ...
            <button className='btn-up' onClick={() => this._choseSortType(3, 'TOTAL_ORDER_MONEY', 'ASC')} />
            <button className='btn-down' onClick={() => this._choseSortType(4, 'TOTAL_ORDER_MONEY', 'DESC')} />
          ...
          <span className='th wid04'>
            ...
            <button className='btn-up' onClick={() => this._choseSortType(5, 'TOTAL_SUCCESS_ORDER_MONEY', 'ASC')} />
            <button className='btn-down' onClick={() => this._choseSortType(6, 'TOTAL_SUCCESS_ORDER_MONEY', 'DESC')} />
          ...
          <span className='th wid05'>
            ...
            <button className='btn-up' onClick={() => this._choseSortType(7, 'TOTAL_REFUND_ORDER_MONEY', 'ASC')} />
            <button className='btn-down' onClick={() => this._choseSortType(8, 'TOTAL_REFUND_ORDER_MONEY', 'DESC')} />
          ...
          {/*</span>*/}
        </div></ScrollOnTop> : <ScrollOnTop className="scroll-on-top-wap"><div className='table-head'> {/* 订单数表格标题 */}
          <span className='th wid01'>
            <em className='txt'>设备编号</em>
          </span>
          <span className='th wid02'>
        ...
        {(!this.props.error && this.props.list && this.props.list.length === 0)
          ...
          : null}
        {/* 数据列表 */}
        {!this.props.error && this.props.list && this.props.summaryData && this.props.list.length > 0
          ?
                ...
                if (this.state.tabIndex === 'ORDER_MONEY') { // 订单金额列表渲染
                    ...
                    <table className='summary'>
                      <tbody>
                        <tr className='tr clearfix' onClick={i === this.state.showIndex && this.state.isShow ? (e) => this._hide(e, i) : (e) => this._show(e, i, item.shopId)}>
                          ...
                          <td>{(parseFloat(item.totalOrderMoney || 0) / 100).toFixed(2)}</td>
                          <td>{(parseFloat(item.totalSuccessOrderMoney || 0) / 100).toFixed(2)}</td>
                          {/*<td>{(parseFloat(item.totalRefundOrderMoney || 0) / 100).toFixed(2)}</td>*/}
                          <td>{item.totalRefundOrderMoney || 0}<i className='dianfont icon-fenlei' /></td>
                        </tr>
                      </tbody>
                    </table>
                            ...
                            return (
                              <tr className='tr clearfix' key={j}>
                                <td>
                                  ...
                                  <p>{item2.boxNo || 0}</p>
                                  <p>{item2.boxIsOnline === '0' ? <span className='status online green'>充电宝：{item2.boxPowerBankNum || 0} </span> : <span className='status red'>充电宝：{item2.boxPowerBankNum}</span>}</p>
                                ...
                                <td>{item2.totalScanNum}</td>
                                <td>{(parseFloat(item2.totalOrderMoney || 0) / 100).toFixed(2)}</td>
                                <td>{(parseFloat(item2.totalSuccessOrderMoney || 0) / 100).toFixed(2)}</td>
                                <td>{(parseFloat(item2.totalRefundOrderMoney || 0) / 100).toFixed(2)}</td>
                                {/*<td>{(parseFloat(item2.deviceAvgOrderDayMoney || 0) / 100).toFixed(2)}</td>*/}
                              </tr>
                            )
                          })
                          }
                  ...
                  return <div key={i} className='device-cell'>
                    ...
                    <div className='shop clearfix' onClick={() => router.push('/shops/' + item.shopId || 0)}>
                      <div className='install'>
                        <p className='install-shop-name'>{item.shopName || ''}{(item.shopType === '0' || item.shopType === '1') ? <i className='shop-type zhiying'>直营</i> : item.shopType ? <i className='shop-type daili'>代理</i> : null}</p>
                        <p className='install-info'>员工：{item.shopSeller || ''} &nbsp; &nbsp;设备总数：{item.shopBoxNum || 0} &nbsp;&nbsp;充电宝：{item.shopPowerBankNum || 0}</p>
                      </div>
                    </div>
                    <table className='summary'>
                      <tbody>
                          ...
                          {this.props.deviceList && this.props.deviceList.map((item2, j) => {
                            return (
                              <tr className='tr clearfix' key={j}>
                                ...
                                {/* <td>{parseFloat(item2.deviceAvgOrderDay || 0).toFixed(2)}</td> */}
                              </tr>
                            )
                          })
                          }
