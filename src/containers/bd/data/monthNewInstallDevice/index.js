...
class Container extends Component {
      ...
      <div>
        ...
        {this.props.summaryData && !this.props.error
          ? <div className='summary'>
            <span className='num'>{this.props.summaryData.newInstallDeviceNum || 0}</span>
            <span className='desc'>绩效新安装设备数</span>
        ...
        <ScrollOnTop className="scroll-on-top-wap">
          <div className='shop-data-head'>
            ...
            <span className='th wid02'>
              <em className='txt'>新安装<br />门店数</em>
              <button className='btn-up' onClick={() => this._choseSortType(3, 'NEW_INSTALL_SHOP_NUM', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(4, 'NEW_INSTALL_SHOP_NUM', 'DESC')} />
            ...
            <span className='th wid03'>
              <em className='txt'>新安装<br />设备数</em>
              <button className='btn-up' onClick={() => this._choseSortType(5, 'NEW_INSTALL_DEVICE_NUM', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(6, 'NEW_INSTALL_DEVICE_NUM', 'DESC')} />
            ...
            <span className='th wid04'>
              <em className='txt'>总安装<br />设备数</em>
              <button className='btn-up' onClick={() => this._choseSortType(7, 'PERF_DEVICE_NUM', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(8, ' PERF_DEVICE_NUM', 'DESC')} />
            ...
            <span className='th wid05'>
              <em className='txt'>总设备成<br />功订单</em>
              <button className='btn-up' onClick={() => this._choseSortType(9, 'SUCCESS_ORDERR_NUM', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(10, 'SUCCESS_ORDERR_NUM', 'DESC')} />
        ...
        {/* 数据列表 */}
        {this.props.list && !this.props.error && this.props.list.length > 0
        ? this.props.list.map((item, i) => {
          let date = new Date().getFullYear() + '-' + item.time
          return <div className='shop-data-line' key={i}>
          {/*return <div className='shop-data-line' onClick={!isBd ? () => router.push('/bd/data/monthNewInstallDevice/detail?date=' + date + '&role=' + this.props.location.query.role) : null} key={i}>*/}
            <span className='td wid01'>{item.time}</span>
            <span className='td wid02'>{item.newInstallShopNum}</span>
            <span className='td wid03'>{item.newInstallDeviceNum}</span>
            <span className='td wid04'>{item.deviceNum}</span>
            <span className='td wid05'>
              {item.successOrderNum}
  ...
  _choseCity (level1, level2, level3) {
    // alert('1级：'+JSON.stringify(level1)+'&&&&&&&&&&2级：'+JSON.stringify(level2)+'&&&&&&&&&&3级：'+JSON.stringify(level3))
    this._closeCity()
    router.replace({
      pathname: location.pathname,
      query: assign({}, this.props.location.query, {
