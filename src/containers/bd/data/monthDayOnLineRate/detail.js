...
class monthDayOnLineRateDetailContainer extends Component {
  ...
  componentDidMount () {
    Bridge.setNavTitle('绩效-日均设备在线率详情')
      ...
      <div>
        ...
        <ScrollOnTop className="scroll-on-top-wap">
          <div className='shop-data-head'>
            ...
            <span className='th wid03'>
              ...
              <button className='btn-up' onClick={() => this._choseSortType(3, 'ONLINE_DEVICE', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(4, 'ONLINE_DEVICE', 'DESC')} />
            ...
            <span className='th wid04'>
              ...
              <button className='btn-up' onClick={() => this._choseSortType(5, 'AVERAGE_ONLINE', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(6, 'AVERAGE_ONLINE', 'DESC')} />
        ...
        ? this.props.list.map((item, i) => {
          return <div className='shop-data-line' key={i}>
            ...
            <span className='td wid03'>{item.averageOnline}</span>
            <span className='td wid04'>{item.averageOnline}</span>
          </div>
        }) : null}
      </div>
    )
    ...
    switch (this.props.location.query.role) {
      case 'ceo':
        this.props.actions.fetchMonthData({
          ...
          perfSortField: this.state.sortDesc || 'PERF_DEVICE_NUM',
...
export default connect(mapStateToProps, mapDispatchToProps)(monthDayOnLineRateDetailContainer)
