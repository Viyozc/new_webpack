      ...
      <div>
        ...
        {this.props.summaryData && !this.props.error
         ? <div className='summary'>
           <span className='num'>{(parseFloat(this.props.summaryData.averageOnline || 0) * 100).toFixed(2)}%</span>
           <span className='desc'>绩效设备日均在线率</span>
        ...
        <ScrollOnTop className="scroll-on-top-wap">
          <div className='shop-data-head'>
            ...
            <span className='th wid02'>
              <em className='txt'>安装设<br />备总数</em>
            ...
            <span className='th wid03'>
              <em className='txt'>在线设<br />备数</em>
              <button className='btn-up' onClick={() => this._choseSortType(5, 'ONLINE_DEVICE', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(6, 'ONLINE_DEVICE', 'DESC')} />
            ...
            <span className='th wid04'>
              <em className='txt'>日均在<br />线率</em>
              <button className='btn-up' onClick={() => this._choseSortType(7, 'AVERAGE_ONLINE', 'ASC')} />
              <button className='btn-down' onClick={() => this._choseSortType(8, 'AVERAGE_ONLINE', 'DESC')} />
        ...
        ? this.props.list && this.props.list.map((item, i) => {
          ...
          {/*return <div className='shop-data-line' onClick={!isBd ? () => router.push('/bd/data/monthDayOnLineRate/detail?date=' + date + '&role=' + this.props.location.query.role) : null} key={i}>*/}
            <span className='td wid01'>{item.time}</span>
            <span className='td wid02'>{item.deviceNum}</span>
            <span className='td wid03'>{item.onlineNum}</span>
            <span className='td wid04'>
              {parseFloat(item.averageOnline || 0).toFixed(2)}
